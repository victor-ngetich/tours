	...
	def clean(self,*args,**kwargs):
		...
		if user_qs.count() == 1:
			...
			if not user.is_active:
				raise forms.ValidationError("This user is not active, check your email and click on the activation link we provided to activate the user.")
		...
		if user.groups.filter(name='Tourist').exists()==False and user.groups.filter(name='Tour Leader').exists()==False:
